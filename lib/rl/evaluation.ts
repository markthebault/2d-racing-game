...
export type EvaluationSummary = { version: number; targetLaps: number; track: number; runs: EvaluationRun[]; successRate: number; meanScore: number; meanProgress: number; meanOffroadTime: number; meanLapTime: number | null };
export function summarizeEvaluation(track: number, runs: EvaluationRun[], targetLaps = 1): EvaluationSummary {
  ...
  return { version: EVALUATION_VERSION, targetLaps, track, runs: runs.map(r => ({ ...r })), successRate: laps.length / runs.length,
...
export function betterEvaluation(candidate: EvaluationSummary, incumbent: EvaluationSummary | null) {
  ...
  if (candidate.track !== incumbent.track || candidate.version !== incumbent.version || candidate.targetLaps !== incumbent.targetLaps) throw new Error('Cannot rank evaluations from different tracks or suites.');
...
export class EvaluationSuite {
  ...
  index = 0;
  constructor(track: number, preset: Preset, targetLaps = 1) { this.environment = new DrivingEnvironment(track, preset, false, targetLaps); this.environment.reset(EVALUATION_STARTS[0]); }
  ...
  summary() { return summarizeEvaluation(this.environment.track, this.runs, this.environment.targetLaps); }
}
