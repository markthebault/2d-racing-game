...
export type EvaluationSummary = { version: number; track: number; runs: EvaluationRun[]; successRate: number; meanScore: number; meanProgress: number; meanOffroadTime: number; meanLapTime: number | null };
export function summarizeEvaluation(track: number, runs: EvaluationRun[]): EvaluationSummary {
  ...
  return { version: EVALUATION_VERSION, track, runs: runs.map(r => ({ ...r })), successRate: laps.length / runs.length,
    meanScore: mean('score'), meanProgress: mean('progress'), meanOffroadTime: mean('offroadTime'),
    meanLapTime: laps.length ? laps.reduce((sum, r) => sum + r.time, 0) / laps.length : null };
}
export function betterEvaluation(candidate: EvaluationSummary, incumbent: EvaluationSummary | null) {
  if (!incumbent) return true;
  if (candidate.track !== incumbent.track || candidate.version !== incumbent.version) throw new Error('Cannot rank evaluations from different tracks or suites.');
...
export class EvaluationSuite {
  ...
  index = 0;
  constructor(track: number, preset: Preset) { this.environment = new DrivingEnvironment(track, preset); this.environment.reset(EVALUATION_STARTS[0]); }
  ...
  summary() { return summarizeEvaluation(this.environment.track, this.runs); }
}
