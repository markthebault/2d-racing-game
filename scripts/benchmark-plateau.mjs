...
const seed=Number(process.argv[3]||42),preset=process.argv[4]||'adaptive',output=process.argv[5]||'/tmp/plateau-candidate.json';
const agent=new DQNAgent(seed,true),tracks=[0,1,2],random=seededRandom(seed+101),budget=Number(process.argv[2]||60000),envs=tracks.map(t=>new DrivingEnvironment(t,preset,false,3));
...
for(let track=0;track<5;track++){const suite=new EvaluationSuite(track,preset,3);while(!suite.done)suite.step(s=>agent.act(s));results.push(suite.summary());}
await writeFile(output,JSON.stringify({seed,preset,budget,episode,seconds:(Date.now()-started)/1000,results},null,2));
await writeFile(output.replace('.json','-weights.json'),JSON.stringify(agent.exportWeights()));
