...
export function TrainingLab({ track, laps, onFrame, onFleet }: { track: number; laps: number; onFrame: (frame: AgentFrame) => void; onFleet: (frame: FleetFrame | null) => void }) {
  ...
  return <section className="training-lab" aria-label="Reinforcement learning lab">
    ...
    <details className="model-library">
      ...
      <p>Loading starts a fresh evaluation on this circuit. Then run the model or continue training here. Source weights are retained. A successful load does not guarantee a completed race.</p>
...
function TrainingSession({ track, laps, config, onFrame, onFleet, onSave }: { track: number; laps: number; config: Session; onFrame: (frame: AgentFrame) => void; onFleet: (frame: FleetFrame | null) => void; onSave: (model: Checkpoint) => void }) {
  ...
  useEffect(() => {
    ...
    instance.postMessage({ type: 'init', track, laps: initialLaps.current, checkpoint: config.checkpoint, preset: config.preset, seed: config.seed } satisfies WorkerCommand);
  ...
  const busy = ['training', 'evaluating', 'playing', 'replaying'].includes(stats.status), unavailable = ['loading', 'error'].includes(stats.status) || stats.track !== track || stats.targetLaps !== laps;
  ...
  return <>
    ...
    <div className="training-actions">
      <Button className="train-button" disabled={unavailable} onClick={() => command({ type: busy ? 'pause' : 'resume' })}>{busy ? <Pause size={16}/> : <Play size={16}/>} {busy ? 'Pause all' : stats.pausedActivity === 'batch' || stats.pausedActivity === 'play' ? 'Resume all' : stats.steps ? 'Resume training' : 'Start training'}</Button>
    ...
    <div className="batch-summary">
      ...
      <strong>{fleet ? `50 cars · episodes ${fleet.first}–${fleet.last}` : `Next group · ${stats.batchCount}/50 recorded`}</strong>
      ...
      <small>Learning across: {stats.trainedTracks.length ? stats.trainedTracks.map(t => TRACKS[t].name).join(' · ') : 'No circuits yet'}. Switch circuits to continue with the same learner.</small>
    </div>
    ...
    <div className="training-metrics">
      <Metric label="Laps completed" value={`${frame?.completedLaps ?? stats.completedLaps} / ${laps}`}/><Metric label="Checkpoints this lap" value={`${frame?.checkpoints ?? stats.checkpoints} / ${stats.checkpointCount}`}/>
    ...
    <div className="training-explanation"><details><summary>Rewards and inputs in this experiment</summary><dl>
      ...
      </dl><p>36 input slots include the 12 ray distances, speed and controls, relative road position and heading, map headings ahead, and off-road duration. New history inputs expose elapsed time, stalled time, signed progress, progress already rewarded, checkpoint count, relative distance to the next checkpoint, completed-lap fraction, and the requested lap count.</p><p>{rewardConfig.absolutePosition ? 'This comparison stage also uses X/Z and absolute lap-position features.' : 'X/Z and absolute lap-position features are masked to zero. Relative task progress remains available to help predict rewards. The rolling motion window is not included in the model inputs.'} This is still map-assisted driving because road headings ahead are included.</p><p>Progress cannot earn points twice. Leaving the road for one second, moving more than three units beyond its edge, moving less than the selected minimum in one second, six seconds without forward progress, or reaching 90 seconds per requested lap ends an attempt. Normal playback starts at the finish line. Varied-start evaluations require a full circuit back to each test’s own starting position.</p>
      {frame && <p className="reward-now">Latest decision: checkpoint {frame.reward.checkpoint.toFixed(1)} · progress {frame.reward.progress.toFixed(1)} · speed {frame.reward.speed.toFixed(1)} · off-road {frame.reward.offroad.toFixed(1)} · reverse {frame.reward.reverse.toFixed(1)} · time {frame.reward.time.toFixed(1)} · finish {frame.reward.finish} · failure {frame.reward.failure}</p>}</details>
      <details><summary>How evaluation and saving work</summary><p>Five fixed starts test the line, lateral and heading offsets, and two other positions around the circuit. Learning and exploration are off throughout evaluation. Best models are ranked by lap completion first, then mean reward within this experiment and circuit.</p><p>Evaluations run after episode 1 and every 10 episodes. Best-score reports appear every 100 episodes. Evaluation uses the latest learner unless you press Evaluate best. Lap time is the average per lap in completed races; a dash means no completed lap. Off-road time averages all five runs.</p><p>Switching circuits retains the live learner and experience memory, and measures scores again on the destination. Loading a saved model instead starts a separate session with a fresh experience buffer. Refreshing preserves saved weights but not optimizer state or unfinished training.</p></details></div>
